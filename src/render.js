...
export function renderSvg(definitions) {
  ...
  for (const pe of planeElements) {
    ...
    if (pe.$type === 'bpmndi:BPMNEdge') {
      const points = (pe.waypoint || []).map((wp) => `${wp.x},${wp.y}`).join(' ');
      parts.push(
        `<polyline points="${points}" fill="none" stroke="#444" stroke-width="1.5" marker-end="url(#arrow)"/>`
      );
      const label = element.name || (element.conditionExpression && element.conditionExpression.body);
      if (label) {
        const wps = pe.waypoint || [];
    ...
    if (type.endsWith('Event')) {
      ...
      const defs = (element.eventDefinitions || []).map((d) => d.$type.replace('bpmn:', '').replace('EventDefinition', ''));
      if (defs.length) {
        parts.push(
